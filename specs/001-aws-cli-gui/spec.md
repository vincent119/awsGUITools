...
- **FR‑013**：AWS Profile 自動載入：從 `~/.aws/config` 與 `~/.aws/credentials` 讀取可用 profiles，無需額外 config.yaml 設定。
- **FR‑014**：Profile/Region 聯動：切換 AWS Profile 時，自動帶入該 profile 設定的預設 region。
- **FR‑015**：i18n 多語言支援：介面文字支援英文（預設）與繁體中文，可透過設定或快捷鍵切換。
- **FR‑016**：S3 物件瀏覽：進入 S3 Bucket 後可瀏覽內部物件與目錄結構，支援：
  - 以 `/` 分隔的 prefix 導航（類似目錄結構）
  - 顯示物件名稱、大小、最後修改時間、StorageClass
  - 支援返回上層目錄
- **FR‑017**：Route53 支援：新增 Route53 資源類型，支援：
  - 列出所有 Hosted Zones
  - 進入 Hosted Zone 後顯示所有 DNS Records
  - Record 詳情包含 Type、Name、Value、TTL
- **FR‑018**：狀態列快捷鍵提示：在狀態列顯示常用快捷鍵（?:Help、q:Quit），方便使用者快速參考。
...
- 在地化：UI 支援 i18n 多語言（英文為預設，繁體中文為次選）；專有名詞保留英文字形；語言可透過設定或 runtime 切換。
...
- **S3Object**：key、size、lastModified、storageClass、isDirectory(prefix)
- **Route53HostedZone**：id、name、recordCount、isPrivate、comment
- **Route53Record**：name、type、ttl、values[]、aliasTarget(optional)
- **Relations**：
...
- **SC‑007（設定）**：啟動時自動讀取 `~/.aws/config` 中的 profiles，切換 profile 時自動帶入對應 region。
- **SC‑008（i18n）**：所有 UI 文字可切換為英文或繁體中文；預設語言為英文。
- **SC‑009（S3 瀏覽）**：可進入 S3 Bucket 瀏覽物件與目錄，支援 prefix 導航與返回上層。
- **SC‑010（Route53）**：可列出 Hosted Zones 並查看各 Zone 的 DNS Records。
